...
class CliFlowTests(unittest.TestCase):
    ...
    def test_run_self_heal_regenerates_drifted_wrappers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            controller_root = Path(tmp_dir) / "controller"
            repos_root = Path(tmp_dir) / "managed"
            controller_root.mkdir(parents=True)
            ...
            with patch("repo_autowork.cli.build_config", return_value=config), patch(
                ...
                "repo_autowork.cli.safe_sync_crontab"
            ), patch("sys.argv", ["repo-autowork", "run", "--repos-root", str(repos_root), "--dry-run", "--self-heal"]), patch(
                "sys.stdout", stdout
            ):
                exit_code = cli.main()
    ...
    def test_run_self_heal_json_reports_wrapper_contracts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            controller_root = Path(tmp_dir) / "controller"
            repos_root = Path(tmp_dir) / "managed"
            controller_root.mkdir(parents=True)
    ...
    def test_telegram_sync_self_heal_json_includes_per_project_healing_details(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("wrapper_contracts", data)
            self.assertTrue(data["wrapper_contracts"]["controller_healed"])
            self.assertIn("healed_paths", data["wrapper_contracts"])
            self.assertTrue(len(data["wrapper_contracts"]["healed_paths"]) > 0)

    def test_history_until_filters_by_upper_timestamp(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir) / "controller"
            root.mkdir(parents=True)
            ...
            with patch("repo_autowork.cli.build_config", return_value=config), patch(
                "repo_autowork.cli.load_state", return_value=state
            ), patch("sys.argv", ["repo-autowork", "history", "--repos-root", str(root.parent), "--until", "2026-05-27T12:00:00+00:00", "--json"]), patch("sys.stdout", stdout):
                exit_code = cli.main()

            self.assertEqual(exit_code, 0)
            data = json.loads(stdout.getvalue())
            self.assertEqual(data["total_rounds"], 2)
            self.assertEqual(data["until"], "2026-05-27T12:00:00+00:00")
            timestamps = [r["timestamp"] for r in data["rounds"]]
            self.assertNotIn("2026-05-27T14:00:00+00:00", timestamps)

    def test_history_since_and_until_filter_range(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            root = Path(tmp_dir) / "controller"
            root.mkdir(parents=True)
            ...
            config = build_config(root, repos_root=str(root.parent))
            history = [
                TelegramSyncSummary(handled=1, ignored={}, dispatch_outcomes=[], timestamp="2026-05-27T08:00:00+00:00"),
                TelegramSyncSummary(handled=2, ignored={}, dispatch_outcomes=[], timestamp="2026-05-27T10:00:00+00:00"),
                TelegramSyncSummary(handled=3, ignored={}, dispatch_outcomes=[], timestamp="2026-05-27T12:00:00+00:00"),
                TelegramSyncSummary(handled=4, ignored={}, dispatch_outcomes=[], timestamp="2026-05-27T14:00:00+00:00"),
            ]
            state = State(telegram_sync_history=history)
            stdout = io.StringIO()
            ...
            with patch("repo_autowork.cli.build_config", return_value=config), patch(
                "repo_autowork.cli.load_state", return_value=state
            ), patch("sys.argv", ["repo-autowork", "history", "--repos-root", str(root.parent), "--since", "2026-05-27T10:00:00+00:00", "--until", "2026-05-27T12:00:00+00:00", "--json"]), patch("sys.stdout", stdout):
                exit_code = cli.main()
            ...
            self.assertEqual(data["total_rounds"], 2)
            timestamps = [r["timestamp"] for r in data["rounds"]]
            self.assertNotIn("2026-05-27T08:00:00+00:00", timestamps)
            self.assertNotIn("2026-05-27T14:00:00+00:00", timestamps)
